...
public class GameBoard {
	...
	public GameBoard(int boardSize, Queue<Player> players1) {
		this.boardSize=boardSize;
		this.board= new char[(2*boardSize)-1] [(2*boardSize)-1]; // As board has to take size considering | , - & symbols of o *  as for 3*3 , 5*5 is needed
...
//			if(count>=(2*boardSize ) && checkEndGame(p,row,col)) break;
			if(checkEndGame(p,row,col)) break;
			nextTurn.offer(p);
		}
	}
	...
	private int getUserInput(Player p){
		printBoard();
		System.out.println(p.getPlayerName() + " Please Enter a number between 1 - "+ (boardSize*boardSize) +" Your Symbol Is :"+p.getPlayerSymbol());
